...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("✔️").info("Git Client Found")
    except GitCommandError:
        LOGGER("✔️").info("Invalid Git Command")
        ...
        LOGGER("✔️").info("Fetched Latest Updates")
...
async def create_botlog(client):
    if HAPP is None:
        return
    LOGGER("✔️").info(
        "Membuat GroupLog Untuk Anda...."
    )
    desc = "Group Log untuk Hyper Userbot.\n\n✨ Powered By ~ @HyperSupportQ ✨"
    try:
        gruplog = await client.create_supergroup("Logs Hyper Userbot", desc)
        if await in_heroku():
